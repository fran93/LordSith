...
public class CivilService {
  ...
  public static final int MIN_EXPLORER = 2;
  public static final int MIN_DESTROYER = 2;
  public static final int MIN_TRANSPORTER_SMALL = 2500;
  ...
  public void deployFleet() {
    ...
    try {
      if (fleetService.isThereAFleet() && fleetService.isFleetAvailable()) {
        ...
        int countSmallCargo = fleetService.numberOfShips(TechnologyEnum.KLEINER_TRANSPORTER.getId());
        ...
        if (countKreuzer > MIN_FLEET_TO_DEPLOY || countSchlachtKreuzer > MIN_FLEET_TO_DEPLOY || countZerstorer > MIN_FLEET_TO_DEPLOY || countPathfinder > MIN_FLEET_TO_DEPLOY
          ...
          if (countPathfinder > 0) {
            int quantity = countPathfinder - MIN_EXPLORER;
            if (quantity > 0) {
              firefox.get().findElement(By.name("explorer")).sendKeys(String.valueOf(quantity));
            }
          }
          if (countZerstorer > 0) {
            int quantity = countZerstorer - MIN_DESTROYER;
            if (quantity > 0) {
              firefox.get().findElement(By.name("destroyer")).sendKeys(String.valueOf(quantity));
            }
          }
          if (countSmallCargo > 0) {
            int quantity = countSmallCargo - MIN_TRANSPORTER_SMALL;
            if (quantity > 0) {
              firefox.get().findElement(By.name("transporterSmall")).sendKeys(String.valueOf(quantity));
            }
          }
      ...
      log.info("deployFleet: " + ex.getMessage());
    }
  }
