...
public class ManageFleetService {
		...
		if (canContinue("continueToFleet2")) {
			...
			if (canContinue("continueToFleet3")) {
				...
				if (canContinue("sendFleet")) {
					...
					firefox.get().findElement(By.xpath("//li[@data-msg-id=" + id + "]"))
							.findElement(By.className("icon_refuse")).click();
					firefox.loading();
				}
			}
		}
	...
	private int getNecesaryFleet(List<WebElement> rows) {
		StringBuilder title = new StringBuilder(
				rows.get(2).findElement(By.className("tooltipClose")).getAttribute("title"));
	...
	private boolean isStatusOn(int id) {
		return firefox.get().findElement(By.xpath(LI_DATA_TECHNOLOGY + id + "]")).getAttribute("data-status")
				.equals(StatusEnum.ON.getValue());
	}

	private int numberOfShips(int id) {
		return Integer.parseInt(firefox.get().findElement(By.xpath(LI_DATA_TECHNOLOGY + id + "]"))
